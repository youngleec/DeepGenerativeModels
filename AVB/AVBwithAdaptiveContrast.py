...
def QNetwork(data, eps):
    ...
    mean, var = tf.nn.moments(QNet_layer2, 0)

    var = tf.pow(var, 0.5)

    return mean, var, QNet_layer2, QParameter
...
def LatentPlot(sess, j, num = 5):
    ...
    batch, _ = mnist.train.next_batch(1000)
    noise = SampleN(1000, noise_dim)
    mean, var = sess.run([Q_mean, Q_var], {data: batch, eps: noise})

    z_prior = np.random.normal(mean, var, (2 * num, z_dim))
...
Q_mean, Q_var, QNet_z, QParameter = QNetwork(data, eps)

QNet_z_norm = (QNet_z - Q_mean) / Q_var
...
REUSE = True
T_q_logit, T_q, _ = TNetwork(data, QNet_z_norm)
...
VAE_loss = - tf.reduce_mean(- T_q) - log_likelihood - 0.5 * tf.reduce_sum(tf.pow(QNet_z_norm, 2.0))
