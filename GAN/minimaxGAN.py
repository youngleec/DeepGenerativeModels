...
def Discriminator(x):
    ...
    Dlogit, DNlayer2 = fc_op(DNlayer1, 1, 'DNlayer2', DParameter, tf.nn.sigmoid, reuse = REUSE)

    return Dlogit, DNlayer2, DParameter
...
D_real_logit, D_real, D_Parameter = Discriminator(real_data)

REUSE = True
D_fake_logit, D_fake, _ = Discriminator(fake_data)

# Minimax Loss

#D_Loss = -tf.reduce_mean(tf.log(tf.clip_by_value(D_real, 1e-8, 1.0)) + tf.log(tf.clip_by_value(1.0 - D_fake, 1e-8, 1.0)))
#G_Loss = tf.reduce_mean(tf.log(tf.clip_by_value(1.0 - D_fake, 1e-8, 1.0)))
...
G_Loss = - tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_fake_logit, labels=tf.zeros_like(D_fake_logit)))
